# Delivery Service (shipping-service module)

**Phase 3 (HLD):** Consumes **`payments`** (`com.ecommerce.payment.processed`), creates **`AWAITING_SHIPMENT`** in DB. REST marks **SHIPPED** / **DELIVERED** → **`deliveries`** topic. **Redis** idempotency.

## Features

- ✅ **Payment → delivery row** (no Kafka until REST)
- ✅ **REST**: `GET /api/deliveries?status=AWAITING_SHIPMENT`, `PATCH /api/deliveries/{orderId}/status`
- ✅ **Kafka**: `com.ecommerce.delivery.shipped` / `com.ecommerce.delivery.delivered` on **`deliveries`**
- ✅ **Redis** idempotency (payment + REST)
...
```
PaymentProcessed (payments) → DB row AWAITING_SHIPMENT
PATCH status SHIPPED → deliveries topic (shipped)
PATCH status DELIVERED → deliveries topic (delivered)
```
